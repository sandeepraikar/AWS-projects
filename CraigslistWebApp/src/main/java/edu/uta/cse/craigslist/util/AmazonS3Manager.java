...
public class AmazonS3Manager {
	...
	public static ObjectMetadata getObjectMetadata(String key){
		...
		try{
			s3object = s3Client.getObject(new GetObjectRequest(Constants.AWS_S3_BUCKET_NAME, key));
	...
	public static URL getImageURL(String key) {
		URL url=null;
		try{
			s3Client.setObjectAcl(Constants.AWS_S3_BUCKET_NAME,key,
					CannedAccessControlList.PublicRead);
			GeneratePresignedUrlRequest request = new GeneratePresignedUrlRequest(
					Constants.AWS_S3_BUCKET_NAME,key);
			request.setExpiration(new Date(2015, 05, 30));
